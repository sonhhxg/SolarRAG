...
# logger.info("Elasticsearch version: "+str(elasticsearch.__version__))
...
class ESConnection:
    ...
    def conn(self):
        for _ in range(2):
            try:
                ...
                logger.error("Fail to connect to es: " + str(e))
                time.sleep(1)
                ...
                try:
                    if not self.version():
                        ...
                        r = self.es.update(
                            ...
                            refresh=True,
                            retry_on_conflict=100)
                    logger.info("Successfully upsert: %s" % id)
                    T = True
                    break
                except Exception as e:
                    logger.warning("Fail to index: " +
    ...
    def bulk(self, df, idx_nm=None):
        ...
        for _ in range(100):
            try:
                ...
                logger.warn("Fail to bulk: " + str(e))
                if re.search(r"(Timeout|time out)", str(e), re.IGNORECASE):
                    time.sleep(3)
                    continue
                self.conn()
    ...
    def rm(self, d):
            ...
            try:
                if not self.version():
                    ...
                    r = self.es.delete(
                        ...
                        refresh=True,
                        doc_type="_doc")
                logger.info("Remove %s" % d["id"])
                return True
            except Exception as e:
                logger.warn("Fail to delete: " + str(d) + str(e))
                if re.search(r"(Timeout|time out)", str(e), re.IGNORECASE):
                    time.sleep(3)
                    continue
        ...
        logger.error("Fail to delete: " + str(d))
    ...
    def updateByQuery(self, q, d):
        ...
        for i in range(3):
            try:
                ...
                logger.error("ES updateByQuery exception: " +
    ...
    def deleteByQuery(self, query, idxnm=""):
        for i in range(3):
            try:
                ...
                logger.error("ES updateByQuery deleteByQuery: " +
    ...
    def indexExist(self, idxnm):
        ...
        for i in range(3):
            try:
                ...
                logger.error("ES updateByQuery indexExist: " + str(e))
                if str(e).find("Timeout") > 0 or str(e).find("Conflict") > 0:
                    continue
    ...
    def docExist(self, docid, idxnm=None):
        for i in range(3):
            try:
                ...
                logger.error("ES Doc Exist: " + str(e))
                if str(e).find("Timeout") > 0 or str(e).find("Conflict") > 0:
                    continue
        return False
    ...
    def createIdx(self, idxnm, mapping):
        try:
            ...
            logger.error("ES create index error %s ----%s" % (idxnm, str(e)))

    def deleteIdx(self, idxnm):
        try:
            return self.es.indices.delete(idxnm, allow_no_indices=True)
        except Exception as e:
            logger.error("ES delete index error %s ----%s" % (idxnm, str(e)))
    ...
    def scrollIter(self, pagesize=100, scroll_time='2m', q={
        ...
        for _ in range(100):
            try:
                ...
                logger.error("ES scrolling fail. " + str(e))
                time.sleep(3)

        sid = page['_scroll_id']
        scroll_size = page['hits']['total']["value"]
        logger.info("[TOTAL]%d" % scroll_size)
        # Start scrolling
        while scroll_size > 0:
