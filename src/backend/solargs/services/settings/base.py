...
from solargs.services.settings.constants import VARIABLES_TO_GET_FROM_ENVIRONMENT
...
class Settings(BaseSettings):
    ...
    telemetry_base_url: str = "https://solargs.gateway.scarf.sh"
    transactions_storage_enabled: bool = True
    """If set to True, Langflow will track transactions between flows."""
    vertex_builds_storage_enabled: bool = True
    """If set to True, solargs will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "127.0.0.1"
    """The host on which solargs will run."""
    port: int = 7860
    """The port on which solargs will run."""
    workers: int = 1
    """The number of workers to run."""
    log_level: str = "critical"
    """The log level for solargs."""
    log_file: str | None = "logs/solargs.log"
    """The path to log file for solargs."""
    ...
    """If set to True, solargs will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, solargs will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which solargs will auto save flows."""
    ...
    def set_dev(cls, value):
        from solargs.utils.settings import set_dev
    ...
    def set_database_url(cls, value, info):
        if not value:
            logger.debug("No database_url provided, trying SOLARGS_DATABASE_URL env variable")
            if solargs_database_url := os.getenv("SOLARGS_DATABASE_URL"):
                value = solargs_database_url
                logger.debug("Using SOLARGS_DATABASE_URL env variable.")
            else:
                ...
                from solargs._version import get_solargs_version
                from solargs._version import is_pre_release as solargs_is_pre_release

                version = get_solargs_version()
                is_pre_release = solargs_is_pre_release(version)
                ...
                if info.data["save_db_in_config_dir"]:
                    ...
                    logger.debug(f"Saving database to solargs directory: {database_dir}")

                pre_db_file_name = "solargs-pre.db"
                db_file_name = "solargs.db"
    ...
    def set_components_path(cls, value):
        if os.getenv("SOLARGS_COMPONENTS_PATH"):
            logger.debug("Adding SOLARGS_COMPONENTS_PATH to components_path")
