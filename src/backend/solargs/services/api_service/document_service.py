...
from solargs.api.utils import current_timestamp, get_format_time, get_uuid
...
from solargs.api.apps.db import FileType, TaskStatus, ParserType, LLMType,StatusEnum
from solargs.api.apps.db.db_models import Document,DB,Knowledgebase,Tenant,UserTenant,Task
from solargs.api.apps.db.db_utils import bulk_insert_into_db
from solargs.rag.db_conn.redis_conn import REDIS_CONN
    ...
    def update_progress(cls):
        ...
        for d in docs:
            try:
                ...
                if str(e).find("'0'") < 0:
                    logger.info("fetch task exception")
...
def queue_raptor_tasks(doc):
    ...
    task["type"] = "raptor"
    assert REDIS_CONN.queue_product(CFG.SVR_QUEUE_NAME, message=task), "Can't access Redis. Please check the Redis' status."
