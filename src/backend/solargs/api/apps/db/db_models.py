...
from solargs.api import utils
from solargs._private.config import Config
from solargs.api.apps.db import SerializedType, ParserType
...
class LongTextField(TextField):
    field_type = TextFieldType[CFG.DATABASE_TYPE.upper()].value
...
class BaseDataBase:
    def __init__(self):
        database_config = CFG.DATABASE.copy()
        db_name = database_config.pop("name")
        self.database_connection = PooledDatabase[CFG.DATABASE_TYPE.upper()].value(db_name, **database_config)
...
DB.lock = DatabaseLock[CFG.DATABASE_TYPE.upper()].value
...
class User(DataBaseModel):
    ...
    def get_id(self):
        jwt = Serializer(secret_key=CFG.SECRET_KEY)
