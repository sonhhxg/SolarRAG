...
# if TYPE_CHECKING:
#     from solargs.api.apps.db.db_models import User
#     from solargs.services.auth.utils import get_current_active_user
#
...
def bs64_encoded(text):
    utf8_str = str(text).encode("utf-8")
    base64_str = base64.b64encode(utf8_str).decode(
        "utf-8")
    return base64_str

def bs64_decoded(text):
